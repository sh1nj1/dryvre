...
해커톤 확장에서는 사람이 여러 Agent 블록 중 하나를 명시적으로 골라 Local Codex CLI를 실행할 수 있다. Agent와 Skill의 정본은 계속 블록과 참조이며, 실행 상태만 얇은 runtime projection으로 분리한다. 자동 위임이나 트리거 loop는 도입하지 않는다. 상세 계약은 [로컬 Agent와 계층형 Skill 스펙](agent-runtime-spec.md)을 따른다.

...
서버, 클라이언트와 MCP가 같은 블록 타입과 작은 연산 집합을 공유한다. 세 가지 뷰를 위해 별도 엔티티를 만들지 않고, 리치텍스트 이중 직렬화·채팅의 이중 온톨로지가 다시 생길 자리를 구조적으로 줄인다.
...
현재 범위에서 `rank`가 있는 자식만 재귀 렌더링한다. 스트림 블록에 `rank`를 부여하면 ID, 자식과 참조를 유지한 채 문서에 나타난다. 다시 `null`로 만들면 스트림으로 돌아간다.
...
계층적 권한 관리는 해커톤 MVP 아키텍처에 포함하지 않는다. 현재 스키마와 연산 프로토콜은 블록별 grant, 상속 또는 권한 판정을 요구하지 않으며, 데모는 신뢰할 수 있는 단일 사용자 환경에서 실행한다. 권한 모델을 활성 범위로 승격할 때 스키마, 서버 판정, 조회 노출 규칙과 테스트를 함께 설계한다. 후보 사항은 [계층적 권한 관리 TODO](not-implemented/hierarchical-permissions.md)에만 기록한다.
...
- CRDT·로컬 퍼스트 동기화 엔진: 해커톤 수직 슬라이스에 비해 동기화 복잡도가 큼

...
- 상태: Local Agent 런타임 Phase 0~3 및 managed Dryvre MCP 구현 완료, 고정 작업 루프 확장 활성
...
프롬프트는 shell 인자가 아니라 stdin으로 전달한다. stdout의 JSONL을 파싱하고 마지막 `agent_message`를 대상 블록 아래의 새 스트림 블록으로 저장한다. Real run은 managed Codex home의 base `config.toml`로 Dryvre MCP를 연결해 실행 중에도 정본 블록을 읽고 생성·편집할 수 있다.
...
Agent run event는 run을 요청한 subject의 WebSocket에만 전송한다. 일반 block `applied` event는 기존 공동 트리 동기화 계약대로 연결 전체에 전달한다. 브라우저는 App root의 WebSocket 하나를 공유하며 Agent composer가 별도 연결을 만들지 않는다.
...
`DRYVRE_AGENT_WORKSPACES`의 name→path JSON map에 허용한 실제 경로만 workspace로 등록한다. Agent config의 `workspace`는 서버 설정의 이름을 참조하며 사용자 입력 경로를 직접 받지 않는다. 실행 전 `realpath`가 허용 루트 안인지 다시 검사한다.
...
서버는 managed Codex home의 base `config.toml`에 빌드된 Dryvre MCP entrypoint를 필수 stdio MCP로 기록한다. 세 Dryvre 도구는 이 비대화형 전용 MCP에서 `default_tools_approval_mode = "approve"`로 실행하고, 각 도구는 read-only·destructive·idempotent·closed-world 힌트를 함께 제공한다. 사용자 `config.toml`은 복사하거나 변경하지 않으며 model, reasoning과 sandbox는 기존 CLI 인자로 전달한다. 이 방식은 stock Codex 0.144.6의 실제 `exec` MCP registry에서 검증한다. Agent subject의 짧은 세션을 run timeout 범위로 발급해 `DRYVRE_SESSION`으로만 전달하고 run 종료 시 DB session을 삭제한다. 따라서 MCP가 만든 block op도 기존 `/api/ops`와 op log를 통과하며 Agent subject 작성자로 기록된다. 이 세션 처리는 기능적 identity 귀속을 위한 MVP 구현이며 프로덕션 보안 경계로 간주하지 않는다.

해커톤 기본 sandbox는 `workspace-write`로 고정하고 approval policy는 `never`로 실행한다. 브라우저가 임의 CLI 인자나 MCP server를 추가할 수 없고 server가 등록한 세 Dryvre MCP 도구만 노출하므로, non-interactive run에서 대화형 승인 대기 없이 이 도구를 실행한다. `danger-full-access`와 `--dangerously-bypass-approvals-and-sandbox`는 UI와 API에서 제공하지 않는다. sandbox 또는 권한으로 거부된 동작은 실패로 반환하고 사용자가 로컬에서 직접 수행하도록 안내한다.
...
9. real run 동안 managed Dryvre MCP가 대상 subtree를 다시 읽거나 요청 범위 안의 block을 생성·편집할 수 있게 한다.
...
- 실시간 상태: WebSocket event를 우선 사용하고 느린 reconciliation polling을 병행해 연결 장애와 초기 event race에서도 완료 상태를 복구
...
### Phase 4a — Managed Dryvre MCP (구현 완료)

- run 전용 managed Codex home의 base config로 Dryvre MCP를 필수 stdio server로 주입한다.
- 짧은 Agent subject session으로 MCP block read/write의 실행 identity를 유지한다.
- 최종 Agent 메시지는 기존 runtime이 결과 stream block으로 저장하고, MCP가 만든 추가 block은 기존 op log와 WebSocket 흐름을 그대로 사용한다.
...
- real MCP smoke: `npm run test:smoke:codex`로 stock Codex가 `dryvre_read_tree`와 `dryvre_create_block`을 호출해 ephemeral DB에 nonce block을 남기는지 확인

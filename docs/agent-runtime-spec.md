...
프롬프트는 shell 인자가 아니라 stdin으로 전달한다. stdout의 JSONL을 파싱하고 마지막 `agent_message`를 대상 블록 아래의 새 스트림 블록으로 저장한다. Real run은 managed Codex profile로 Dryvre MCP를 연결해 실행 중에도 정본 블록을 읽고 생성·편집할 수 있다.
...
서버는 `dryvre-managed.config.toml` profile에 필수 stdio MCP로 빌드된 Dryvre MCP entrypoint를 주입한다. Agent subject의 짧은 세션을 run timeout 범위로 발급해 `DRYVRE_SESSION`으로만 전달하고 run 종료 시 DB session을 삭제한다. 따라서 MCP가 만든 block op도 기존 `/api/ops`와 op log를 통과하며 Agent subject 작성자로 기록된다. 이 세션 처리는 기능적 identity 귀속을 위한 MVP 구현이며 프로덕션 보안 경계로 간주하지 않는다.
...
- run 전용 Codex profile에 Dryvre MCP를 필수 stdio server로 주입한다.

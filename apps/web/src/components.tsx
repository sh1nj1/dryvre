...
export function BoardView({ blocks, messages, selectedId, draggedBlockId, onSelect, onStatus, onDragStart, onDragEnd }: { blocks: DryvreBlock[]; messages: BlockMessage[]; selectedId: string; draggedBlockId: string | null; onSelect: (id: string) => void; onStatus: (id: string, status: TaskStatus) => void; onDragStart: (id: string) => void; onDragEnd: () => void }) {
  ...
  // The board only renders blocks in the current scope, so a drop that changes a
  // block outside that scope would mutate hidden data while the card never appears.
  // Reject those drops (no highlight, no status write) instead of losing the block.
  const draggingInScope = draggedBlockId != null && blocks.some((block) => block.id === draggedBlockId);

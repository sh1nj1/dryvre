...
export function StreamComposer({ selected, agents, target, live, liveMessage, onSend, onSent }: { selected: DryvreBlock; agents: Block[]; target: Block | undefined; live: boolean; liveMessage: WsServerMessage | undefined; onSend: (body: string) => void; onSent: (targetId: string, resultBlockId?: string) => void }) {
  ...
  return <div className="composer stream-composer">
    ...
    {agentSelected && <div className="agent-composer-status">
      <div className={`agent-readiness ${readiness?.ready ? 'ready' : 'not-ready'}`}><i />{readinessLabel}<span>{skillNames.length ? `${skillNames.length} skills` : 'No skills'}</span>{readiness?.error && <small>{agentError(readiness.error)}</small>}</div>
    ...
    <div className="composer-actions"><span className="context-chip">◎ {selected.title}</span><button className="tool-pill">@ Reference</button>{agents.length > 0 && target && <select className="composer-mode" aria-label="Send as" value={agentId} onChange={(event) => setAgentId(event.target.value)} disabled={Boolean(busy)}><option value="">Message</option>{agents.map((agent) => <option value={agent.id} key={agent.id}>{(agent.bodyMd ?? '').match(/^#\s+@agent\s+([^\n]+)/)?.[1] ?? 'Agent'}</option>)}</select>}<button className={agentSelected ? 'agent-run-btn' : 'send-btn'} aria-label={agentSelected ? 'Run Agent' : 'Send message'} disabled={!value.trim() || Boolean(busy) || (agentSelected && (!readiness?.ready || !target))} onClick={() => void send()}>{busy ? 'Running' : agentSelected ? 'Run' : '↑'}</button></div>
  </div>;
}
